...
@section('content')
    <h3 class="mb-3">Edit Template</h3>
    <form method="POST" action="{{ route('templates.update', $template) }}">
        ...
        <div class="mb-3">
            ...
            <select class="form-select" name="trigger_type">
                <option value="days_before_expiry" selected>Days Before Expiry</option>
            </select>
        </div>
        <div class="mb-3">
            <label class="form-label">Days Before</label>
            <input class="form-control" type="number" name="days_before" value="{{ old('days_before', $template->days_before) }}" min="0" required>
        </div>
        <div class="mb-3">
            <label class="form-label">Message Body</label>
            <textarea class="form-control" name="message_body" rows="4" required>{{ old('message_body', $template->message_body) }}</textarea>
